...
c//onsole.log('Inrouter ');
// calls from Web Pages, Gets and Posts

//Customer GETS
//get list of Customer (GetAll)

router.get('/getcustlisttest', async (req, res, next) => {
    const fetch = require('node-fetch');
        //console.log('hit it');  
...
.then((data) => {  // if call is successful
  ...
  retresposne = res.render('getcustlisttest', { resultdata:  data} )
})
...
console.log('rightbefore call');
 //get list of Customer By id (GetById)
router.get('/getcustlistbyidtest', async (req, res, next) => {
  console.log('r',router.all);  
  const fetch = require('node-fetch');

  console.log('hit it getcustlistbyidtest'); 
...
//console.log('golbal:',  global.DB_token);
retresposne = res.render('getcustlistbyidtest', { resultdata:  data} )
})
.catch((error) => { // if call returns any status other than a 200 level status
